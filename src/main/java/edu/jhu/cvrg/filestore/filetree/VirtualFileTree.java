...
public class VirtualFileTree extends FileTree {
	...
	protected void buildTree() {
		...
		try {
			...
			if(rootFolderId > 0L){
				...
				this.treeRoot = new FileNode(null, ROOT_FOLDER_NAME, rootFolderId, true, EnumFileStoreType.VIRTUAL_DB, null);
	...
	private void exploreNode(FileNode treeRoot, Collection<VirtualNodeDTO> children){
		for (VirtualNodeDTO nodeDto : children) {
			FileNode folderNode = new FileNode(treeRoot, nodeDto.getName(), nodeDto.getId(), nodeDto.isFolder(), EnumFileStoreType.VIRTUAL_DB, null);
